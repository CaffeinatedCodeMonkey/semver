...
const printAffectedResponse = {
  tasks: [],
  projects: ['demo', 'demo-e2e'],
  projectGraph: {
    nodes: ['demo', 'lib1', 'lib2', 'npm:@mock/npm-lib1', 'npm:@mock/npm-lib2'],
    dependencies: {
      demo: [
        {
          type: 'static',
...
describe('projectDependencies', () => {
  ...
  it('returns a list of libs that the project is dependent on', async () => {
    mockExecAsync.mockReturnValue(
      of({ stderr: undefined, stdout: JSON.stringify(printAffectedResponse) })
    );
    ...
    expect(mockExecAsync).toBeCalledWith('npm', [
      'run',
      '-s',
      'nx print-affected',
    ]);
  ...
  it('handles a failure in retrieving the dependency graph', async () => {
    mockExecAsync.mockReturnValue(
      throwError({ stderr: 'thrown error', stdout: undefined })
    );
