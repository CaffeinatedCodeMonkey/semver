...
export async function getProjectDependencies(
  ...
  return Array.from(
    assembleDependenciesFromGraph(dependencyGraph, projectName)
  );
}

async function getParsedDependencyGraph(): Promise<{
  [key: string]: ProjectGraphDependency[];
}> {
  return new Promise((resolve, reject) => {
    execAsync('npm', ['run', '-s', 'nx print-affected']).subscribe({
      next: (output) =>
        resolve(JSON.parse(output.stdout).projectGraph.dependencies),
